...
#%%
PATH_DATA = "data/v4/preprocessed"
...
all_data = all_data[all_data.expt_block != NB_BLOCK_1STEP]
...
# exclude for tool use
all_data = all_data[~all_data['debrief_tools'].str.contains('yes')]
...
#exclude participants pt 2 - nonlearners
def median_absolute_deviation(x):
    B = 1.4826
    mad = B * np.median(np.absolute(x - np.median(x)))
    threshold = np.median(x) - (2*mad)
    return threshold

cols_plot = ['expt_group', 'expt_block', 'expt_curriculum', 'expt_turker']
df_acc = all_data.groupby(cols_plot, as_index=False)['resp_correct'].apply(lambda x: (np.nansum(x)/len(x))*100)
df_acc = pd.DataFrame(df_acc)
df_mad_thresh = df_acc.groupby(['expt_curriculum', 'expt_block'], as_index=False)['resp_correct'].apply(
        lambda x: median_absolute_deviation(x))

df_acc.loc[df_acc.expt_curriculum=='simple_blocked_magician'].hist(column='resp_correct')

idx_late_blocks = [1.0,3.0,5.0,7.0]
all_data['learner'] = None
for i,row in all_data.iterrows():
    sub_acc = df_acc.loc[(df_acc.expt_turker==row['expt_turker']) & (df_acc.expt_block.isin(idx_late_blocks))]
    mad = df_mad_thresh.loc[(df_mad_thresh.expt_curriculum==row['expt_curriculum']) &  (df_mad_thresh.expt_block.isin(idx_late_blocks))]

    all_data.loc[[i]]['learner'] = not np.any(np.array(sub_acc['resp_correct']) < np.array(mad['resp_correct']))


col_group = ['expt_group', 'expt_curriculum', 'expt_block']
all_data.groupby(col_group)['expt_subject'].nunique()

col_group = ['expt_group', 'expt_curriculum', 'learner']
all_data.groupby(col_group)['expt_subject'].nunique()
...
np_acc = prep.pd2np(all_data, col_group, 'resp_correct') #or: 'resp_correct''resp_reactiontime'
